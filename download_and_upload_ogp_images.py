...
def download_and_upload_ogp_images(sanity_api_token):
    image_asset_mapping = {}
    
    if not sanity_api_token:
        print("Error: Sanity API token was not provided.")
        return
    ...
    for post_id, data in scraped_data.items():
        ...
        if image_url:
            ...
            try:
                ...
                upload_process = subprocess.run(upload_command, capture_output=True, text=True, env=env)
                
                if upload_process.returncode == 0:
                    asset_info = json.loads(upload_process.stdout)
                    asset_id = asset_info['_id']
                    image_asset_mapping[post_id] = {
                        '_type': 'image',
                        'asset': {
                            '_type': 'reference',
                            '_ref': asset_id
...
if __name__ == '__main__':
    ...
    if not token:
        ...
        print("Please set it before running this script: export SANITY_API_TOKEN=your_token")
    else:
        download_and_upload_ogp_images(token)